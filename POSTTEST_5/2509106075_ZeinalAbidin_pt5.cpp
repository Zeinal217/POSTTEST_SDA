...
bool cariBuku(BukuNode *head, string judulDicari)
{
    ...
    do
    {
        // 3. cek apakah temp -> judulbuku sama dengan judulcari. Jika ya return true
        if (temp->judulBuku == judulDicari)
        {
            return true;
        }
        // 4. jika tidak lanjut ke node berikutnya, berhenti jika temp kembali mencapai head
        temp = temp->next;
    } while (temp != head);
...
int main()
{
    ...
    cout << "Mencari buku '" << buku1 << "': " << (cariBuku(head, buku1) ? "Ditemukan" : "Tidak Ditemukan") << endl;
    // Harusnya output: Ditemukan

    cout << "Mencari buku '" << buku2 << "': " << (cariBuku(head, buku2) ? "Ditemukan" : "Tidak Ditemukan") << endl;
